...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.site-settings.update", [$siteSetting->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="value">{{ trans('cruds.siteSetting.fields.value') }}</label>
                <textarea class="form-control {{ $errors->has('value') ? 'is-invalid' : '' }}" name="value" id="value">{{ old('value', $siteSetting->value) }}</textarea>
                @if($errors->has('value'))
                    <div class="invalid-feedback">
                        {{ $errors->first('value') }}
                    </div>
