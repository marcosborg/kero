...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.refund.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.refunds.update", [$refund->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('order') ? 'is-invalid' : '' }}" name="order_id" id="order_id" required>
                    @foreach($orders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('order_id') ? old('order_id') : $refund->order->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_cents') ? 'is-invalid' : '' }}" type="number" name="amount_cents" id="amount_cents" value="{{ old('amount_cents', $refund->amount_cents) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reason') ? 'is-invalid' : '' }}" type="text" name="reason" id="reason" value="{{ old('reason', $refund->reason) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Refund::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $refund->status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('payload') ? 'is-invalid' : '' }}" name="payload" id="payload">{{ old('payload', $refund->payload) }}</textarea>
