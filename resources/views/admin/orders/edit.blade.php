    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.update", [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Order::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $order->status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subtotal_cents') ? 'is-invalid' : '' }}" type="number" name="subtotal_cents" id="subtotal_cents" value="{{ old('subtotal_cents', $order->subtotal_cents) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount_cents') ? 'is-invalid' : '' }}" type="number" name="discount_cents" id="discount_cents" value="{{ old('discount_cents', $order->discount_cents) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('shipping_cents') ? 'is-invalid' : '' }}" type="number" name="shipping_cents" id="shipping_cents" value="{{ old('shipping_cents', $order->shipping_cents) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tax_cents') ? 'is-invalid' : '' }}" type="number" name="tax_cents" id="tax_cents" value="{{ old('tax_cents', $order->tax_cents) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_cents') ? 'is-invalid' : '' }}" type="number" name="total_cents" id="total_cents" value="{{ old('total_cents', $order->total_cents) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('billing_address') ? 'is-invalid' : '' }}" name="billing_address_id" id="billing_address_id">
                    @foreach($billing_addresses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('billing_address_id') ? old('billing_address_id') : $order->billing_address->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" type="text" name="notes" id="notes" value="{{ old('notes', $order->notes) }}">
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Order::PAYMENT_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('payment_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="payment_status_{{ $key }}" name="payment_status" value="{{ $key }}" {{ old('payment_status', $order->payment_status) === (string) $key ? 'checked' : '' }} required>
